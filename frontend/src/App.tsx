...
export default function App() {
  ...
  const loadGameState = useCallback(async () => {
    ...
    try {
      ...
      // Auto-enter the active event if the team hasn't started it yet
      const currentProgress = (data.eventProgress || []).find(
        (p) => p.eventNumber === data.activeEventNumber
      );

      if (!currentProgress) {
        const enterRes = await apiFetch<{ progress: EventProgressData }>(
          '/api/events/enter',
          {
            method: 'POST',
          }
  ...
  const r1Progress = (gameState?.eventProgress || []).find(
    (p) => p.eventNumber === 1
  );

  const r2Progress = (gameState?.eventProgress || []).find(
    (p) => p.eventNumber === 2
  );

  const r3Progress = (gameState?.eventProgress || []).find(
    (p) => p.eventNumber === 3
  );

  const r6Progress = (gameState?.eventProgress || []).find(
    (p) => p.eventNumber === 6
  );
  ...
  const {
   ...
   // Admin route - always render AdminPage for /admin
        ...
        <header className="w-full max-w-6xl mx-auto px-4 py-4 sm:py-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-extrabold tracking-widest text-rose-500 uppercase">
                PRODINNO · technoVIT
              </p>
            </div>
          </div>
      ...
      {window.location.pathname === '/admin' ? (
        ...
        <main className="flex-1 flex flex-col justify-center max-w-6xl w-full mx-auto pb-10 px-2 sm:px-4">
          ...
          {activeEventNumber === 1 && r1Progress && (
            <Event1ForgottenHill
              timeRemainingMs={
                currentProgress?.timeRemainingMs || 30 * 60 * 1000
              }
              ...
              {r2Progress && r2Progress.status !== 'ACTIVE' ? (
                ...
                <div className="w-full flex flex-col items-center animate-fade-in mt-4">
                  ...
                  <div className="mb-4">
                    <TimerBadge
                      initialTimeRemainingMs={
                        currentProgress?.timeRemainingMs ||
                        30 * 60 * 1000
                      }
                      onExpire={handleTimerExpire}
                    />
                  ...
                  <ProgressTracker
                    ...
                    completedStages={completedLevels.map((c) => ({
                      stage: c.stage,
                      name: c.level.name,
                    }))}
                  ...
                  <GuessInput
                    ...
                    justCorrectName={
                      justUnlocked ? justUnlocked.name : null
                    }
                  />
          ...
          {activeEventNumber === 6 && (
            <>
              {r6Progress && r6Progress.status !== 'ACTIVE' ? (
                <FinalScreen
                  round1Score={r2Progress?.score || 0}
                  round2Score={r6Progress.score || 0}
                  totalScore={
                    (r2Progress?.score || 0) +
                    (r6Progress.score || 0)
                  }
